      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {tipoActivo && (
          <a href={buildUrl({ tipo: undefined }, base)}
            className="flex items-center gap-1 bg-teal-50 text-primary-700 text-xs font-bold px-3 py-1.5 rounded-full hover:bg-teal-100 transition-colors shrink-0">
        ...
        <div className="relative" ref={ref}>
          <button onClick={() => setOpen(o => !o)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 text-sm font-bold transition-all ${
              open || activeCount > 0
                ? 'bg-teal-500 text-white border-primary-600'
                  ...
                  {ordenes.map(o => (
                    <a key={o.value} href={buildUrl({ orden: o.value }, base)}
                      ...
                      className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${
                        ordenActivo === o.value ? 'bg-teal-50 border-primary-200 text-primary-700' : 'bg-gray-50 text-gray-700 border-gray-200 hover:border-gray-400'
                      }`}>
                      {o.label}
